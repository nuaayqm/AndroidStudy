...
public class Completion implements Serializable {
    private final String TAG = "com.alibaba.mychatgpt.entity.completions.Completion";
    ...
    public void setMaxTokens(Integer maxTokens) {
        if (maxTokens > 4096) {
            Log.e(TAG,"maxTokens参数异常，不能超过4096");
            this.maxTokens = 4096;
            return;
        }
        if (maxTokens <= 0) {
            Log.e(TAG,"maxTokens参数异常，不能小于0");
            this.maxTokens = 64;
            return;
        }
    ...
    public void setTemperature(double temperature) {
        if (temperature > 2 || temperature < 0) {
            Log.e(TAG,"temperature参数异常，temperature属于[0,2]");
            this.temperature = 2;
            return;
        }
        if (temperature < 0) {
    ...
    public void setLogprobs(Integer logprobs) {
        if (logprobs > 5) {
            Log.e(TAG,"logprobs参数异常，logprobs最大值==5");
            this.logprobs = 5;
            return;
        }
